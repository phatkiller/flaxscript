...
// Copyright (c) 2013 Flaxscript developers
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight == 1) {nSubsidy = 73017 * COIN;} //1% of PoW as an initial deposit in the JDDFYS Treasury + 23017 for Malacylpse, that dirty bastard.
...
static const int64 nInterval = 1; // Flaxscript: retarget every block
...
static const int64 nMaxAdjustDown = 23; // 23% adjustment down
static const int64 nMaxAdjustUp = 17; // 17% adjustment up
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FlaxscriptMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FlaxscriptMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("FlaxscriptMiner started\n");
    ...
    try { loop {
        ...
        printf("Running FlaxscriptMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FlaxscriptMiner terminated\n");
        throw;
    }
}
