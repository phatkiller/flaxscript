...
//int64_t enforceMasternodePaymentsTime = 4085657524; // Flaxscript
...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleFlaxscript(name.begin()+1, name.end());
            if (mapArgs.count(singleFlaxscript) == 0)
                mapArgs[singleFlaxscript] = entry.second;
            name = singleFlaxscript;
        }
...
#else
    const char* pszModule = "flaxscript";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Flaxscript
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Flaxscript
    // Mac: ~/Library/Application Support/Flaxscript
    // Unix: ~/.flaxscript
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Flaxscript";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "flaxscript.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty flaxscript.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override flaxscript.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "flaxscriptd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Flaxscript will not work properly.");
