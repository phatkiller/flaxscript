...
void Shutdown()
{
    ...
    RenameThread("flaxscript-shutoff");
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: flaxscript.conf)") + "\n";
    if (hmm == HMM_BITCOIND)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: flaxscriptd.pid)") + "\n";
    ...
    strUsage += "  -anonymizeflaxscriptamount=<n> " + _("Keep N flaxscript anonymized (default: 0)") + "\n";
...
{
    RenameThread("flaxscript-loadblk");
...
/** Sanity checks
 *  Ensure that Flaxscript is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Flaxscript Core is shutting down."));
...
#endif
    // Make sure only a single Flaxscript process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Flaxscript Core is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Flaxscript version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Flaxscript") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Flaxscript to complete") << "\n";
    ...
    nAnonymizeDarkcoinAmount = GetArg("-anonymizeflaxscriptamount", 0);
    ...
    LogPrintf("Anonymize Flaxscript Amount %d\n", nAnonymizeDarkcoinAmount);
